-- Repair the set-based bulk RPC for databases that already applied
-- 202609030003. The previous version wrapped data-modifying CTEs inside
-- ARRAY(...), which PostgreSQL rejects with SQLSTATE 0A000. Every mutation
-- below is now a top-level INSERT or UPDATE, and the public RPCs run as the
-- authenticated caller so RLS remains the authorization boundary.
