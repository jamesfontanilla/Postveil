...
function PublicHome({ onSignIn, onSignUp }: { onSignIn: () => void; onSignUp: () => void }) {
  return (
    <main className="public-home landing-home">
      ...
      <div className="landing-announcement"><span>Private mail for the domain you already own.</span><a href="#how-it-works">See how it works <ArrowRight size={14} /></a></div>
      <section className="landing-hero" id="top">
        ...
        <div className="landing-hero-visual" aria-label="Postveil inbox preview">
          <div className="landing-hero-visual-head"><span className="mock-window-brand"><span className="mock-window-mark"><img src="/postveil-logo.svg" alt="" /></span> Postveil</span><span className="mock-window-status"><span /> private mailbox</span></div>
      ...
      <nav className="landing-quick-nav" aria-label="Homepage sections"><a href="#product">Product</a><a href="#how-it-works">How it works</a><a href="#privacy-overview">Privacy</a><a href="/terms">Terms</a><a className="landing-quick-cta" href="#top">Create your mailbox <ArrowRight size={14} /></a></nav>
      ...
      <section className="landing-features" id="product"><div className="landing-section-heading"><p className="landing-eyebrow">A better starting point</p><h2>Everything you need to feel at home in your inbox.</h2><p>Postveil keeps the familiar parts of email and makes the domain setup feel human.</p></div><div className="landing-feature-grid"><article><span className="landing-feature-number">01</span><h3>Bring the name</h3><p>Connect a domain you already own with a guided DNS checklist in plain language.</p><a href="#how-it-works">See how it works <ArrowRight size={14} /></a></article><article><span className="landing-feature-number">02</span><h3>Make the address</h3><p>Create hello@, you@, or a shared address without opening a deployment console.</p><a href="#top">Create your mailbox <ArrowRight size={14} /></a></article><article id="privacy"><span className="landing-feature-number">03</span><h3>Keep the room yours</h3><p>Provider credentials stay behind Postveil, with clear privacy controls around your mail.</p><a href="/privacy">Read our privacy promise <ArrowRight size={14} /></a></article></div></section>
      <section className="landing-process" id="how-it-works"><div className="landing-section-heading"><p className="landing-eyebrow">The setup, made simple</p><h2>From a domain name to a working mailbox in a few clear steps.</h2><p>You stay in control of the domain. Postveil handles the handoff, the checks, and the everyday mail desk.</p></div><div className="landing-process-grid"><article><span>01</span><div><h3>Connect your domain</h3><p>Tell us which address you own. We show the exact records to add and explain what each one does.</p></div></article><article><span>02</span><div><h3>Choose your mailbox</h3><p>Pick a local address for yourself, your team, or a shared purpose like hello@ or support@.</p></div></article><article><span>03</span><div><h3>Open the inbox</h3><p>Once the records are verified, your mailbox is ready for sending, receiving, searching, and organizing.</p></div></article></div></section>
      <section className="landing-capabilities"><div className="landing-capabilities-intro"><p className="landing-eyebrow">Made for daily work</p><h2>Quiet on the surface. Thoughtful underneath.</h2><p>Postveil gives your address a proper home, with the details that make email dependable day after day.</p></div><div className="landing-capability-panel"><div className="landing-capability-row"><strong>Inbox organization</strong><span>Focused views, folders, labels, stars, and bulk actions.</span><Check size={16} /></div><div className="landing-capability-row"><strong>Safe reading</strong><span>Protected rendering, remote-image controls, and link inspection.</span><Check size={16} /></div><div className="landing-capability-row"><strong>Clear delivery</strong><span>Delivery status, authentication results, and understandable failures.</span><Check size={16} /></div><div className="landing-capability-row"><strong>Built to grow</strong><span>Quotas, shared addresses, account security, and domain controls.</span><Check size={16} /></div></div></section>
      <section className="landing-privacy" id="privacy-overview"><div><p className="landing-eyebrow">A private place for your mail</p><h2>Your address should feel like yours from the first message.</h2></div><p>Search, organize, compose, and read without learning a new mental model. Postveil is deliberately familiar where it should be—and deliberate about your domain, data, and control where it matters.</p></section>
      <section className="landing-final-cta"><div><p className="landing-eyebrow">Start with the name you already own</p><h2>Give your domain a quieter inbox.</h2></div><button type="button" className="landing-primary" onClick={onSignUp}>Create your mailbox <ArrowRight size={17} /></button></section>
...
type OnboardingState = {
  ...
  domainStatus?: "pending" | "verified" | "ready";
};

type DomainRecord = { name?: string; type?: string; content?: string; priority?: number | null; ttl?: number | null; status?: string; purpose?: string };
...
const ONBOARDING_STEPS = [
  { label: "Purpose", note: "Choose your starting point" },
  { label: "Domain", note: "Name the address you own" },
  { label: "DNS check", note: "Connect mail safely" },
  { label: "Address", note: "Create your first mailbox" },
  { label: "Done", note: "Your next step is clear" },
];
...
function OnboardingWizard({ session, onComplete }: { session: Session; onComplete: () => void }) {
  ...
  async function refreshDomain() {
    ...
    try {
      ...
      setError(refreshError instanceof Error ? refreshError.message : "The domain could not be checked");
    } finally {
      setDnsRefreshBusy(false);
    }
  }
    ...
    <main className="onboarding-shell">
      <section className="onboarding-frame" aria-labelledby="onboarding-title">
        ...
        <div className="onboarding-content">
          <header className="onboarding-header"><div><span className="onboarding-step-count">0{step + 1} / 05</span><strong>{ONBOARDING_STEPS[step].label}</strong><small>{ONBOARDING_STEPS[step].note}</small></div><button type="button" className="text-button onboarding-save-later" onClick={() => void skipSetup()} disabled={busy}>Save and finish later <ArrowRight size={13} /></button></header>
          ...
          <div className="onboarding-body">
            ...
            {step === 2 && <>
              <p className="eyebrow">A SAFE DNS HANDOFF</p><h2>Connect {cleanDomain || "your domain"} to Postveil.</h2><p className="onboarding-lede">You keep your website, registrar, and passwords exactly where they are. Postveil only needs the mail records shown below.</p>
              <div className="onboarding-provider-row"><label className="onboarding-label" htmlFor="dns-provider">Where do you manage DNS?</label><select id="dns-provider" name="dnsProvider" value={dnsProvider} onChange={(event) => { setDnsProvider(event.target.value); setDomainStatusData(null); setDomainStatus("pending"); }}>{DNS_PROVIDERS.map((provider) => <option key={provider} value={provider}>{provider}</option>)}</select></div>
              <div className="registrar-guide"><div className="registrar-guide-head"><div><span className="eyebrow">YOUR REGISTRAR GUIDE</span><h3>{dnsProvider}</h3></div><Globe2 size={20} aria-hidden="true" /></div><p><strong>Open:</strong> {registrarGuide.path}</p><p><strong>Then:</strong> {registrarGuide.where}. Add the TXT verification record and the mail records below.</p><small>{registrarGuide.note}</small><div className="registrar-safety"><ShieldCheck size={14} /><span><strong>No nameserver change.</strong> Keep your current nameservers and website records.</span></div></div>
              <div className="dns-preview"><div className="dns-preview-head"><div><strong>Connection checklist</strong><small>Postveil checks ownership, receiving, and sender authentication separately.</small></div><span className={domainStatus === "ready" ? "verified-badge" : "pending-badge"}>{domainStatus === "ready" ? "Ready" : domainStatus === "verified" ? "DNS found" : "Pending"}</span></div><div className="dns-row"><span className="dns-mark"><Mail size={15} aria-hidden="true" /></span><div><strong>Domain records</strong><small>{domainStatusData?.dnsReady ? "Your public mail records are visible." : "Add every MX, SPF, DKIM, and DMARC record shown below."}</small></div><span>{domainStatusData?.dnsReady ? "Found" : "Next"}</span></div><div className="dns-row"><span className="dns-mark"><ShieldCheck size={15} aria-hidden="true" /></span><div><strong>Postveil receiving service</strong><small>{domainStatusData?.routeReady ? `Receiving is connected to ${domainStatusData.routeTarget || "Postveil"}.` : "Still waiting for Postveil to activate receiving for this domain."}</small></div><span>{domainStatusData?.routeReady ? "Ready" : "Needs activation"}</span></div>{(domainStatusData?.records || domainStatusData?.manualRecords || []).slice(0, 8).map((record, index) => { const recordText = [record.type || "DNS", record.name || "@", record.content || "", record.priority ? `priority ${record.priority}` : ""].filter(Boolean).join("\t"); return <div className="dns-record-row" key={`${record.type}-${record.name}-${record.content}-${index}`}><span>{record.purpose || record.type || "DNS"}</span><code>{record.name || "@"}</code><code>{record.content || ""}{record.priority ? ` · priority ${record.priority}` : ""}</code><button type="button" className="text-button" aria-label={`Copy ${record.purpose || record.type || "DNS"} record`} onClick={() => void copyRecord(record)}>{copiedRecord === recordText ? "Copied" : <><Copy size={14} aria-hidden="true" /> Copy</>}</button></div>; })}<div className="dns-preview-actions"><button type="button" className="text-button" onClick={() => void refreshDomain()} disabled={dnsRefreshBusy || busy}>{dnsRefreshBusy ? "Checking…" : "Check connection"}</button></div></div>
              <div className="onboarding-hint"><ShieldAlert size={17} aria-hidden="true" /><span>DNS changes usually appear within minutes, but can take up to 24–48 hours. Postveil will not enable sending or receiving until its receiving service confirms the connection.</span></div>
              {error && <div className="form-error" role="alert">{error}</div>}
            ...
            {step === 4 && <>
              <div className="onboarding-success-mark"><Check size={24} /></div><p className="eyebrow">{domainStatus === "pending" ? "ADDRESS RESERVED" : "MAILBOX CREATED"}</p><h2>{mailboxAddress || `${mailboxLocal}@${cleanDomain}`} is yours.</h2><p className="onboarding-lede">{domainStatus === "pending" ? "The address is reserved, but sending and receiving stay disabled until the domain connection is complete." : "Your mailbox is created. Postveil will enable mail only after DNS and receiving-service checks both pass."}</p>
              <div className="onboarding-next-list"><div><Check size={16} /><span><strong>{domainStatus === "pending" ? "Address reserved" : "Mailbox created"}</strong><small>{domainStatus === "pending" ? "Verification is required before mail can flow." : "Your address is available in Postveil."}</small></span></div><div className={domainStatus === "ready" ? "ready" : "pending"}><span className="onboarding-number">02</span><span><strong>Connect DNS</strong><small>{dnsProvider} · MX, SPF, DKIM, and DMARC</small></span><span className="pending-badge">{domainStatus === "ready" ? "Ready" : "Next"}</span></div><div><span className="onboarding-number">03</span><span><strong>Send your first message</strong><small>We’ll check delivery and authentication together.</small></span></div></div>
              {notice && <div className="form-notice" role="status">{notice}</div>}
