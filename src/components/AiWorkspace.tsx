...
export default function AiWorkspace() {
  ...
  function useQuickPrompt(nextFeature: AiFeature, nextQuery: string) {
  ...
  return <section className="ai-workspace">
    ...
    <div className="ai-grid">
      <div className="ai-main-column">
        <section className="setting-card ai-card"><div className="card-heading"><div><p className="eyebrow">ON-DEMAND ASSISTANT</p><h3>Ask Postveil</h3></div><Zap size={18} /></div><p className="field-help">AI reads only the mailbox context needed for this request. It cannot send, delete, label, or change mail.</p><div className="ai-quick-prompts"><span>Try a workflow</span><button type="button" onClick={() => useQuickPrompt("natural_language_search", "Find every promise I made")}>Find every promise I made</button><button type="button" onClick={() => useQuickPrompt("inbox_digest", "Show unanswered messages")}>Show unanswered messages</button><button type="button" onClick={() => useQuickPrompt("inbox_cleanup", "Prepare my follow-up list")}>Prepare my follow-up list</button></div><div className="ai-runner"><label>Capability<select value={feature} onChange={(event) => setFeature(event.target.value as AiFeature)}>{AI_FEATURES.filter((item) => enabledFeatures[item]).map((item) => <option key={item} value={item}>{readableFeature(item)}</option>)}{!AI_FEATURES.some((item) => enabledFeatures[item]) && <option value="inbox_digest">Enable a feature below first</option>}</select></label><label>Request<textarea value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Ask a question or describe the draft style" rows={4} /></label><label className="ai-confirm"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} /><span><strong>I confirm this is an on-demand request.</strong><small>No email action will be taken automatically.</small></span></label><button className="primary-button" onClick={() => void runAi()} disabled={busy || !settings.enabled || !enabledFeatures[feature] || !confirmed || !query.trim()}><Play size={14} /> {busy ? "Working…" : "Run AI"}</button></div>{result !== null && <div className="ai-result"><div className="ai-result-head"><strong>Result</strong><button className="text-button" onClick={() => setResult(null)}>Clear</button></div><pre>{typeof result === "string" ? result : JSON.stringify(result, null, 2)}</pre></div>}</section>
