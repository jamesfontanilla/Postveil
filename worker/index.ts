...
async function api(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  const threadActionMatch = url.pathname.match(/^\/api\/threads\/([^/]+)$/);
  if (threadActionMatch && request.method === "PATCH") {
    const threadId = threadActionMatch[1];
    const owned = await dbRequest<JsonRecord[]>(env, `threads?id=eq.${encodeURIComponent(threadId)}&owner_id=eq.${encodeURIComponent(user.id)}&limit=1`);
    if (!owned[0]) return error("Conversation not found", 404);
    const body = (await request.json()) as JsonRecord;
    const patch: JsonRecord = { updated_at: new Date().toISOString() };
    ...
    if (!Object.keys(patch).some((key) => key !== "updated_at")) return error("No conversation change requested");
    const rows = await dbRequest<JsonRecord[]>(env, `threads?id=eq.${encodeURIComponent(threadId)}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify(patch) });
    ...
    return json(rows[0] || patch);
  }
  const threadSplitMatch = url.pathname.match(/^\/api\/threads\/([^/]+)\/split$/);
  if (threadSplitMatch && request.method === "POST") {
    const threadId = threadSplitMatch[1];
    const body = (await request.json()) as JsonRecord;
    const messageId = String(body.messageId || "");
    const source = await dbRequest<JsonRecord[]>(env, `threads?id=eq.${encodeURIComponent(threadId)}&owner_id=eq.${encodeURIComponent(user.id)}&limit=1`);
    const message = await dbRequest<JsonRecord[]>(env, `messages?id=eq.${encodeURIComponent(messageId)}&thread_id=eq.${encodeURIComponent(threadId)}&owner_id=eq.${encodeURIComponent(user.id)}&limit=1`);
    if (!source[0] || !message[0]) return error("Choose a message from this conversation", 404);
    const created = await dbRequest<JsonRecord[]>(env, "threads", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ owner_id: user.id, subject: message[0].subject || source[0].subject || "(no subject)", subject_normalized: source[0].subject_normalized || normalizeSubject(String(message[0].subject || "")), last_message_at: message[0].received_at || message[0].sent_at || new Date().toISOString() }) });
    if (!created[0]) return error("A new conversation could not be created", 500);
    await dbRequest(env, `messages?id=eq.${encodeURIComponent(messageId)}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", body: JSON.stringify({ thread_id: created[0].id, updated_at: new Date().toISOString() }) });
    return json({ ok: true, thread: created[0] });
  }
  const threadMergeMatch = url.pathname.match(/^\/api\/threads\/([^/]+)\/merge$/);
  if (threadMergeMatch && request.method === "POST") {
    const sourceId = threadMergeMatch[1];
    const body = (await request.json()) as JsonRecord;
    const targetId = String(body.targetThreadId || "");
    if (!targetId || targetId === sourceId) return error("Choose a different conversation");
    const threads = await dbRequest<JsonRecord[]>(env, `threads?id=in.(${encodeURIComponent(sourceId)},${encodeURIComponent(targetId)})&owner_id=eq.${encodeURIComponent(user.id)}&select=id`);
    if (threads.length !== 2) return error("Both conversations must belong to this mailbox", 404);
    await dbRequest(env, `messages?thread_id=eq.${encodeURIComponent(sourceId)}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", body: JSON.stringify({ thread_id: targetId, updated_at: new Date().toISOString() }) });
    await dbRequest(env, `threads?id=eq.${encodeURIComponent(sourceId)}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", body: JSON.stringify({ merged_into_thread_id: targetId, updated_at: new Date().toISOString() }) });
    return json({ ok: true, sourceThreadId: sourceId, targetThreadId: targetId });
  }
  ...
  if (senderBlockMatch && request.method === "PATCH") {
    const body = (await request.json()) as JsonRecord;
    const rows = await dbRequest<JsonRecord[]>(env, `sender_blocks?id=eq.${encodeURIComponent(senderBlockMatch[1])}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ enabled: body.enabled !== false }) });
    return json(rows[0] || null);
  }
  ...
  if (request.method === "POST" && url.pathname === "/api/mail/report") {
    const body = (await request.json()) as JsonRecord;
    const messageId = String(body.messageId || "");
    const reportType = body.reportType === "phishing" ? "phishing" : "spam";
    const message = await dbRequest<JsonRecord[]>(env, `messages?id=eq.${encodeURIComponent(messageId)}&owner_id=eq.${encodeURIComponent(user.id)}&limit=1`);
    if (!message[0]) return error("Message not found", 404);
    await dbRequest(env, "message_reports", { method: "POST", body: JSON.stringify({ owner_id: user.id, message_id: messageId, report_type: reportType, details: String(body.details || "").slice(0, 500) }) });
