...
async function provisionSesDomain(env: Pick<Env, "AWS_ACCESS_KEY_ID" | "AWS_SECRET_ACCESS_KEY" | "AWS_SES_REGION" | "AWS_REGION" | "SES_RECEIPT_RULE_SET_NAME" | "SES_INBOUND_BUCKET" | "SES_INBOUND_LAMBDA_ARN">, domain: string): Promise<SesDomainProvisioning> {
  ...
  try {
    ...
    const verifiedForSending = identity.VerifiedForSendingStatus === true || String(identity.VerifiedForSendingStatus || "").toLowerCase() === "true" || verificationStatus === "SUCCESS";
...
function configuredInboundMxTargets(env: Pick<Env, "INBOUND_MX_TARGETS">): string[] {
  return String(env.INBOUND_MX_TARGETS || "")
    .split(/[\s,;]+/)
...
async function cloudflareZoneForDomain(accessToken: string, domain: string, env: Pick<Env, "INBOUND_MX_TARGETS">): Promise<{ zone: CloudflareZone; dnsReady: boolean } | null> {
...
async function promoteVerifiedMailboxes(env: Env, userId: string, domain: string, mailReady: boolean): Promise<void> {
  // Activation is monotonic. A transient DNS/SES probe failure must not
  // revoke a mailbox that was already enabled; only explicit administration
  // may disable its capabilities.
  if (!mailReady) return;
  ...
  await Promise.all(mailboxes.filter((mailbox) => domainOf(mailbox.address) === domain).map(async (mailbox) => {
    ...
    await d1Request(env, `mailboxes?id=eq.${encodeURIComponent(mailbox.id)}&owner_id=eq.${encodeURIComponent(userId)}`, {
      method: "PATCH",
      body: JSON.stringify({ can_send: mailReady, can_receive: mailReady, settings: { ...settings, domain_verification_status: mailReady ? "verified" : "dns_pending", domain_verification_provider: "cloudflare", mx_target_verified: mailReady } }),
    });
  }));
}
...
async function handleCloudflareOAuthCallback(request: Request, env: Env): Promise<Response | null> {
  ...
  const match = await cloudflareZoneForDomain(accessToken, pending.domain, env);
...
async function api(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
  ...
  if (request.method === "PATCH" && url.pathname === "/api/settings") { const body = (await request.json()) as JsonRecord; const allowed = ["theme", "density", "reading_pane", "language", "timezone", "focused_inbox_enabled", "desktop_notifications", "push_subscription", "onboarding_state", "onboarding_completed_at"]; const patch: JsonRecord = { updated_at: new Date().toISOString() }; for (const key of allowed) if (key in body) patch[key] = body[key]; let undoSeconds = normalizeUndoSeconds(objectValue(mailbox?.settings).send_undo_seconds, 0); if ("send_undo_seconds" in body && mailbox) { undoSeconds = normalizeUndoSeconds(body.send_undo_seconds, undoSeconds); const currentMailboxSettings = objectValue(mailbox.settings); await dbRequest(env, `mailboxes?id=eq.${encodeURIComponent(mailbox.id)}&owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", body: JSON.stringify({ settings: { ...currentMailboxSettings, send_undo_seconds: undoSeconds } }) }); } const rows = await dbRequest<JsonRecord[]>(env, `user_settings?owner_id=eq.${encodeURIComponent(user.id)}`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify(patch) }); return json({ ...(rows[0] || patch), send_undo_seconds: undoSeconds }); }
  ...
  if (request.method === "POST" && url.pathname === "/api/attachments") {
    ...
    if (!mailbox) return error("Connect and verify a domain to create your first mailbox", 409);
